...
A formal reference for the current version of Genepop is [Rousset (2008)](https://doi.org/10.1111/j.1471-8286.2007.01931.x).
...
Genepop is now distributed both as an R package, and as stand-alone software. See the [Genepop distribution page](https://kimura.univ-montp2.fr/~rousset/Genepop.htm) for the latter. This documentation describes the use of the executable. The functionalities it describes are available in an R session, using R functions described only in the package documentation. 

Changes since version 4.0
-------------------------
\index{Genepop@\Genepop, differences from previous versions}

Changes since version 4.6.9 that also affect the R package are described in the [NEWS file](https://cran.r-project.org/package=genepop/news.html) of that package. Only changes affecting the stand-alone executable are reported here.
...
Examples and documentation files are included in the R package and are available on the [Genepop distribution page](https://kimura.univ-montp2.fr/~rousset/Genepop.htm).
...
Example sessions
----------------
To reproduce the examples of this session one should copy in a personal directory the examples files found in the `extdata/` subdirectory of the packageor on the [Genepop distribution page](https://kimura.univ-montp2.fr/~rousset/Genepop.htm).
